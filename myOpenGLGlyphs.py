...
class OpenGLGlyphs:
    ...
    def __init__(self):
        ...
        # self.webcam = VideoStream(  # original
        #     src="rtsp://34.168.1.65:554/ISAPI/streaming/channels/101?auth=YWRtaW46QWRtaW5AMTIz").start()
        self.image = cv2.imread('image.bmp')
        ...
        self.cam_matrix, self.dist_coefs, rvecs, tvecs = self.get_cam_matrix("calibration.yaml")
    ...
    def _handle_glyphs(self, image):
        ...
        if ids is not None and corners is not None:
            rvecs, tvecs, _objpoints = aruco.estimatePoseSingleMarkers(corners[0], 0.6, self.cam_matrix,
                                                                       self.dist_coefs)
            # build view matrix
            ...
            view_matrix = np.array([[rmtx[0][0], rmtx[0][1], rmtx[0][2], tvecs[0][0][0]],
                                    ...
                                    [rmtx[2][0], rmtx[2][1], rmtx[2][2], tvecs[0][0][2]+3],
                                    [0.0, 0.0, 0.0, 1.0]])
